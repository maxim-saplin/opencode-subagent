...
```bash
# Start a new session
./.claude/skills/opencode-subagent-v2/scripts/run_subagent.sh \
...
./.claude/skills/opencode-subagent-v2/scripts/status.sh --cwd ".tmp/opencode-psa" --json

# Wait for any completion
./.claude/skills/opencode-subagent-v2/scripts/status.sh --wait --timeout 60 --json --cwd ".tmp/opencode-psa"

# Check specific agent
./.claude/skills/opencode-subagent-v2/scripts/status.sh --name "payments/refactor/plan" --json
```
...
| Aspect              | v1                                  | v2                                      |
|---------------------|-------------------------------------|-----------------------------------------|
| Execution mode      | sync (blocking) + async             | async only                              |
| Status tracking     | PID liveness only                   | JSONL registry + PID liveness           |
| Status values       | (none explicit)                     | scheduled, running, done, unknown       |
| Status query        | (none)                              | `status.sh` with sync + wait modes      |
| Cancellation        | (none)                              | `cancel.sh`                             |
| Session discovery   | title-based polling                 | wrapper writes sessionId on completion  |
| Index format        | TSV (index.tsv, log.tsv)            | JSONL (runs.jsonl)                      |
| Model inheritance   | sync-only with retries              | explicit `--model` or env required      |
