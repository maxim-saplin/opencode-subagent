...
- Status usage cache: `status --json` surfaces `usage` with `messageCount` and `dialogTokens`.
- Diagram: `status --diagram` renders an ASCII overview without blocking.
...
- Status usage: `status --json` shows `usage` with non-zero `dialogTokens` and message count.
